import os
os.environ["OMP_NUM_THREADS"] = "1"
from astropy.cosmology import LambdaCDM
import numpy as np
import ptemcee, h5py
from multiprocessing import Pool
# import my module
sys.path.append(os.path.expanduser('~'))
import sne_functions as sne
from sne_functions import c

nmod = 243
ndim = 8 ; nwalkers = 50 ; ntemps=4 ; nsteps=10000 #
path_savefile = os.getcwd()
savefilename = 'ptemcee8_DES.h5'

save_interval = 500  # Save every 500 steps

save_dir = os.getcwd()   #'/home/camilo/DES/'
path_savefile = os.getcwd()  # '/home/camilo/DES'
data_dir = os.getcwd()  #'/share/storage3/bets/camilo_storage3/DES'

mvecc, zvec_CMB, zvec_HEL, rdirs, cov_pan_fil = sne.DESY5_data()

def logprior(theta):
    OL, Om, Ob, sig8, h0, Mag, gamma, sigv = theta
    Ok = 1-Om-OL
    #################### priors
    if not (4*Ok**3 + 27*OL*Om**2 > 0): return -np.inf

    if not (-0.6<Ok<0.6)    : return -np.inf
    if not (0<OL<1.5)       : return -np.inf
    if not (0<Om<1.5)       : return -np.inf 
    if not (0.005<Ob<0.2)   : return -np.inf
    if not (Om>Ob)          : return -np.inf
    if not (0<sig8<2)       : return -np.inf
    if not (0.4<h0<1)       : return -np.inf
    if not (-21<Mag<-18)    : return -np.inf
    if not (-1<gamma<3)     : return -np.inf
    if not (0<sigv<325)   : return -np.inf
    #### prior abs Mag
    Magmean = -19.253 ; sigMag = 0.029
    priorMag =  ((Mag-Magmean)/sigMag)**2
    #### prior BBN
    prior_bbn = ((Ob*h0*h0 - 0.02196) / 0.00063)**2
    return -0.5 * priorMag - 0.5 * prior_bbn

def logprob_8(theta):
    OL, Om, Ob, sig8, h0, Mag, gamma, sigv = theta
    #################### theory - model (Astropy)
    cosmo = LambdaCDM(H0=h0*100, Om0=Om, Ode0=OL, Ob0=Ob)
    lum_dis = cosmo.comoving_transverse_distance(zvec_CMB).value*h0*100/c*(1+zvec_HEL)
    vecdif = mvecc - 5*np.log10(lum_dis) - (Mag-5*np.log10(100*h0/c) +25)

    cov_tot = cov_pan_fil + sne.cov_PV_mod_8(theta,zvec_CMB, rdirs, lum_dis,nmod)

    if np.all( np.linalg.eigvals(cov_tot) > 0):
        pass
    else: return -np.inf
    
    inv_cov_tot = np.linalg.inv(cov_tot)
    signo, log_det_tot = np.linalg.slogdet(cov_tot)
    inv_L_tot_fil = np.linalg.cholesky(inv_cov_tot)
    ####################

    mahalanobis = np.linalg.multi_dot([vecdif,inv_L_tot_fil,inv_L_tot_fil.T,vecdif])
    return -0.5 * mahalanobis - 0.5 * log_det_tot

# Parameter-specific initialization
def create_custom_initial_positions(ntemps, nwalkers, ndim):
    """
    Create initial positions with different distributions for each parameter
    OL, Om, Ob, sig8, h0, Mag, gamma, sigv
    """
    initial_positions = np.zeros((ntemps, nwalkers, ndim))

    initial_positions[:, :, 0] = np.random.normal(0.65, 0.05, size=(ntemps, nwalkers))
    initial_positions[:, :, 1] = np.random.normal(0.35, 0.05, size=(ntemps, nwalkers))
    initial_positions[:, :, 2] = np.random.normal(0.042, 0.005, size=(ntemps, nwalkers))
    initial_positions[:, :, 3] = np.random.uniform(0.4, 1, size=(ntemps, nwalkers))
    initial_positions[:, :, 4] = np.random.normal(0.73, 0.001, size=(ntemps, nwalkers))
    initial_positions[:, :, 5] = np.random.normal(-19.253, 0.03, size=(ntemps, nwalkers))
    initial_positions[:, :, 6] = np.random.uniform(0.4, 0.6, size=(ntemps, nwalkers))
    initial_positions[:, :, 7] = np.random.normal(100, 10, size=(ntemps, nwalkers))
    
    return initial_positions

print('test de likelihood:')
print(logprob_8([0.65,0.35,0.042,0.8,0.73,-19.253,0.545,240]))



with Pool() as pool:
    print("Running Parallel Tempered MCMC...")

    sampler = ptemcee.Sampler(nwalkers, ndim, logprob_8, logprior,
                            ntemps=ntemps, pool=pool)  
    
    initial_positions = create_custom_initial_positions(ntemps, nwalkers, ndim)
    # Run in chunks of save_interval steps
    total_chunks = nsteps // save_interval
    for chunk in range(total_chunks):
        print(f"Running chunk {chunk + 1}/{total_chunks} ({save_interval} steps)...")
        
        # Run the sampler for save_interval steps
        sampler.run_mcmc(initial_positions, save_interval)
        
        # Save progress to HDF5 file
        print(f"Saving progress to {savefilename}...")
        sne.save_to_h5(os.path.join(path_savefile, savefilename), sampler, save_interval)

        # Update initial positions to the current state for the next chunk
        initial_positions = sampler.chain[:, :, -1, :]  # Last position of each walker