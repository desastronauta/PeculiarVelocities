...
ndim = 7 ; nwalkers = 60 ; nsteps=30000 ; save_interval = 5000 
path_savefile = "/share/storage3/bets/camilo_storage3/Pantheon/"
savefilename = 'emcee_pp_semPV_comDESI.h5'

mvecc, zvec_CMB, zvec_HEL, rdirs, cov_pan_fil = sne.Pantheon_data()

def logprior(theta):
    Ok, Om, Ob, sig8, H0, M, gamma = theta
    OL = 1-Om-Ok
    #################### priors
    ...
    #### prior BBN
    h = H0 / 100
    prior_bbn = ((Ob*h*h - 0.02196) / 0.00063)**2
...
def logprob_SN(theta):
    Ok, Om, Ob, sig8, H0, M, gamma = theta
    OL = 1-Om-Ok
    h = H0 / 100
    #################### theory - model (Astropy)
    cosmo = LambdaCDM(H0=H0, Om0=Om, Ode0=OL, Ob0=Ob)
    lum_dis = cosmo.comoving_transverse_distance(zvec_CMB).value*H0/c*(1+zvec_HEL)
...
def chi2_fs8(theta_fs8, redshifts=fs8_data['z']):
    ...
    fs8_theory = interp_fsigma8(redshifts)
...
def logprob(theta):
    ...
    if not np.isfinite(lp):
        return -np.inf
    return lp + logprob_SN(theta) + (-0.5 * chi2_fs8([Om, Ok, sig8, gamma]))
...
p0 = [[ 0.00 + 0.05 * np.random.randn(),
        ...
        73 + 10 * np.random.uniform(low=-1, high=1),
